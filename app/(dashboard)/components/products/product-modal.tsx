        ...
        <Modal isOpen={isOpen} onClose={onClose} title={isEditMode ? "Edit Product" : "Add New Product"}>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <div className="flex gap-7">
                    ...
                    <div className="flex flex-col gap-4 w-full">
                        <div className="input-group-admin">
                            ...
                            <input type="text" id="name" name="name" placeholder="e. g. Running Shoes" value={formData.name} onChange={handleChange} />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="input-group-admin">
                                <label htmlFor="price">Price (IDR)</label>
                                <input type="number" id="price" name="price" placeholder="e. g. 50000" value={formData.price} onChange={handleChange} />
                            </div>
                            <div className="input-group-admin">
                                <label htmlFor="stock">Stock</label>
                                <input type="number" id="stock" name="stock" placeholder="e. g. 50" value={formData.stock} onChange={handleChange} />
                            </div>
                        </div>
                        <div className="input-group-admin">
                ...
                <div className="input-group-admin">
                    ...
                    <textarea id="description" name="description" rows={7} placeholder="Product Details..." value={formData.description} onChange={handleChange} />
                </div>
                <Button className="ml-auto mt-2 rounded-lg" type="submit" onClick={handleSubmit} disabled={isSubmitting}>{isEditMode ? "Update Product" : "Create Product"}</Button>
            </form>
        </Modal>
    )
}
