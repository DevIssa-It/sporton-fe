            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <div className="flex gap-7">
                    <div className="min-w-60">
                        <ImageUploadPreview label="Category Image" className="h-65" value={ImagePreview} onChange={
                            (file) => {
                                setImageFile(file);
                    ...
                    <div className="flex flex-col gap-4 w-full">
                        <div className="input-group-admin">
                            ...
                            <input type="text" id="name" name="name" placeholder="e. g. Running" value={formData.name} onChange={handleChange} />
                        </div>
                        <div className="input-group-admin">
                            <label htmlFor="description">Description</label>
                            <textarea id="description" name="description" rows={7} placeholder="Category Details..." value={formData.description} onChange={handleChange} />
                        </div>
                    </div>
                </div>
                <Button className="ml-auto mt-2 rounded-lg" type="submit" onClick={handleSubmit} disabled={isSubmitting}>{isEditMode ? "Update Category" : "Create Category"}</Button>
            </form>
        </Modal>
    )
}
